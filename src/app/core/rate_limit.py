...
def client_ip(scope_or_request: Scope | Request) -> str:
    """The peer address. Deliberately the raw peer, not ``X-Forwarded-For``:
    trusting that header without a configured trust boundary lets any caller
    forge a fresh identity per request and erase their own limit. Wiring a
    proxy-trust boundary is the standing Part 3 deferral.
    """
    if isinstance(scope_or_request, Request):
        return scope_or_request.client.host if scope_or_request.client else "unknown"
...
def rate_limit(
    *, key_prefix: str, limit: int, window_seconds: int
    ...
    async def _check(request: Request, tenant: TenantDep) -> None:
        ...
        key = f"ratelimit:{key_prefix}:{tenant.id}:{client_ip(request)}"
        try:
...
class GlobalRateLimitMiddleware:
    ...
    async def __call__(self, scope: Scope, receive: Receive, send: Send) -> None:
        ...
        key = f"ratelimit:global:{client_ip(scope)}"
        try:
        ...
        logger.info("global_rate_limited", ip=client_ip(scope))
