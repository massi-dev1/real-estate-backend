...
| Rate limits | ✅ | Part 28: global per-IP budget above tenant resolution + per-endpoint auth limits; sliding-window log, real `Retry-After`. Per-surface capture limits since Part 8. Audit SEC-01: keys on the real client behind a proxy — `X-Forwarded-For` is trusted only from a peer inside `TRUSTED_PROXY_CIDRS`, counted from the right. **Set `TRUSTED_PROXY_CIDRS` for your topology when deploying** (prod compose sets the Caddy bridge); leaving it empty behind a proxy pools every client into one budget. |
